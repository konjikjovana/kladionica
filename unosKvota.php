...
$mecevi = $baza->vratiMeceve();

$ishodi = $baza->vratiIshode();
    ...
    <section class="popular-courses-area section-padding-100-0" style="background-image: url(img/core-img/texture.png);">
        <div class="container">
            <div class="row">
                ...
                <div class="col-12">
                      <form method="POST" action="kontroler.php?metoda=unosKvote">
                          <label for="mec">Mec</label>
                          <select class="form-control" name="mec" id="mec">
                              <?php
                              foreach ($mecevi as $mec){
                                  ?>
                              <option value="<?= $mec->mecID?>"><?= $mec->domacin . " " . $mec->gost ?></option>
                              <?php
                              }
                              ?>
                          </select>
                          <label for="ishod">Ishod</label>
                          <select class="form-control" name="ishod" id="ishod">
                              <?php
                              foreach ($ishodi as $ishod){
                                  ?>
                                  <option value="<?= $ishod->ishodID?>"><?= $ishod->nazivIgre . ": " . $ishod->ishod ?></option>
                                  <?php
                              }
                              ?>
                          </select>
                          <label for="kvota">Kvota</label>
                          <input type="text" class="form-control" id="kvota" name="kvota">
                          <hr>
                          <input type="submit" value="Unesi kvotu" class="btn btn-lg btn-dark form-control" name="unosKvote">
                      </form>
                    <br>
                </div>
            </div>
